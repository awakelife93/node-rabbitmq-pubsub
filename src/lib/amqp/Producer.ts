...
class Producer {
    ...
    publishMessage(exchange: string, routingKey: string, message: string, options: Options.Publish = {}): boolean {
        return this.queue.publish(exchange, routingKey, this.generateMessage(message), this.generateOptions(options));
    }

    // * Puts a message directly into the queue.
    sendMessage(queueName: string, message: string, options: Options.Publish = {}): boolean {
        return this.queue.sendToQueue(queueName, this.generateMessage(message), this.generateOptions(options));
    }
}
