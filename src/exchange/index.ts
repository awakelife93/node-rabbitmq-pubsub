...
const initialize = async (): Promise<Replies.AssertExchange[]> => {
  ...
  const deadLetterExchange: Replies.AssertExchange = await createExchange(config.DEAD_LETTER_EXCHANGE);
  ...
  const senderExchange: Replies.AssertExchange = await createExchange(config.SEND_MESSAGE_EXCHANGE);
...
const createExchange = async (exchange: string): Promise<Replies.AssertExchange> => {
