...
const initialize = async (): Promise<Replies.AssertQueue[]> => {
  ...
  const deadLetterQueue: Replies.AssertQueue = await createQueue(config.DEAD_LETTER_QUEUE_NAME);
  ...
  const defaultQueue: Replies.AssertQueue = await createQueue(config.DEFAULT_QUEUE_NAME);
  ...
  const defaultExchangeMessageSendQueue: Replies.AssertQueue = await createQueue(config.EXCHANGE_SEND_MESSAGE_QUEUE_NAME);
